...
const VisualizationButton = ({ type }) => (
...
class VisualizationWizard extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  availableCharts = ['area', 'bar', 'line', 'pie'];
    ...
    return (
      <Modal show onHide={onHide} keyboard>
        ...
        <Modal.Body style={{margin: '10px', 'text-align': 'center'}}>
          {this.state.charts.map(type => <VisualizationButton key={type} type={type}/>)}
